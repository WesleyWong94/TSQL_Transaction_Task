...
IF OBJECT_ID('PURCHASEORDER5362') IS NOT NULL
DROP TABLE PURCHASEORDER5362;

IF OBJECT_ID('INVENTORY5362') IS NOT NULL
DROP TABLE INVENTORY5362;

IF OBJECT_ID('ORDERLINE5362') IS NOT NULL
DROP TABLE ORDERLINE5362;

IF OBJECT_ID('ORDER5362') IS NOT NULL
DROP TABLE [ORDER5362];

IF OBJECT_ID('AUTHORISEDPERSON5362') IS NOT NULL
DROP TABLE AUTHORISEDPERSON5362;

IF OBJECT_ID('ACCOUNTPAYMENT5362') IS NOT NULL
DROP TABLE ACCOUNTPAYMENT5362;

IF OBJECT_ID('CLIENTACCOUNT5362') IS NOT NULL
DROP TABLE CLIENTACCOUNT5362;

IF OBJECT_ID('PRODUCT5362') IS NOT NULL
DROP TABLE PRODUCT5362;

IF OBJECT_ID('LOCATION5362') IS NOT NULL
DROP TABLE [LOCATION5362];

IF OBJECT_ID('GENERALLEDGER5362') IS NOT NULL
DROP TABLE GENERALLEDGER5362;

GO

CREATE TABLE GENERALLEDGER5362(
    ITEMID INTEGER,
...
INSERT INTO GENERALLEDGER5362 (ITEMID, DESCRIPTION, AMOUNT) VALUES
(1, 'ASSETSCASH', 100000.00),
(2, 'ASSETSSTOCK', 0),
(3, 'ASSETSACCOUNT', 0);

CREATE TABLE [LOCATION5362](
...
CREATE TABLE CLIENTACCOUNT5362(
...
CREATE TABLE ACCOUNTPAYMENT5362(
    ACCOUNTID INTEGER,
    DATETIMERECEIVED DATETIME,
    AMOUNT MONEY NOT NULL,
    CONSTRAINT PK_ACCOUNTPAYMENT PRIMARY KEY(ACCOUNTID, DATETIMERECEIVED),
    CONSTRAINT FK_ACCOUNTPAYMENT_ACCOUNT FOREIGN KEY (ACCOUNTID) REFERENCES CLIENTACCOUNT5362,
    CONSTRAINT CHK_ACCOUNTPAYMENT_AMOUNT CHECK(AMOUNT >0)
);

CREATE TABLE AUTHORISEDPERSON5362(
    ...
    CONSTRAINT FK_AUTHORISEDPERSON_CLIENTACCOUNT FOREIGN KEY(ACCOUNTID) REFERENCES CLIENTACCOUNT5362,
    CONSTRAINT CHK_AUTHORISEDPERSON_EMAIL CHECK(EMAIL LIKE '%@%')
);

CREATE TABLE [ORDER5362](
    ...
    CONSTRAINT FK_ORDER_AUTHORISEDPERSON FOREIGN KEY(USERID) REFERENCES AUTHORISEDPERSON5362,
    CONSTRAINT CHK_ORDER_TOTAL CHECK(TOTAL >= 0)
);


CREATE TABLE ORDERLINE5362(
    ORDERID INTEGER,
    PRODUCTID INT,
    ...
    CONSTRAINT FK_ORDERLINE_ORDER FOREIGN KEY(ORDERID) REFERENCES [ORDER5362],
    CONSTRAINT FK_ORDERLINE_PRODUCT FOREIGN KEY(PRODUCTID) REFERENCES PRODUCT5362,
    CONSTRAINT CHK_ORDER_DISCOUNT CHECK(DISCOUNT >=0 AND DISCOUNT <= 0.25),
    CONSTRAINT CHK_ORDERLINE_SUBTOTAL CHECK(SUBTOTAL > 0)
);

CREATE TABLE INVENTORY5362(
    PRODUCTID INT,
    LOCATIONID NVARCHAR(8),
    NUMINSTOCK INTEGER NOT NULL,
    CONSTRAINT PK_INVENTORY PRIMARY KEY(PRODUCTID, LOCATIONID),
    CONSTRAINT FK_INVENTORY_PRODUCT FOREIGN KEY(PRODUCTID) REFERENCES PRODUCT5362,
    CONSTRAINT FK_INVENTORY_LOCATION FOREIGN KEY(LOCATIONID) REFERENCES LOCATION5362,
    CONSTRAINT CHK_INVENTORY_NUMINSTOCK CHECK(NUMINSTOCK >=0)
);

CREATE TABLE PURCHASEORDER5362(
    PRODUCTID INT,
    LOCATIONID NVARCHAR(8),
    ...
    CONSTRAINT FK_PURCHASEORDER_PRODUCT FOREIGN KEY(PRODUCTID) REFERENCES PRODUCT5362,
    CONSTRAINT FK_PURCHASEORDER_LOCATION FOREIGN KEY(LOCATIONID) REFERENCES LOCATION5362,
...
BEGIN
    ...
    INSERT INTO LOCATION5362(LOCATIONID, LOCNAME, ADDRESS, MANAGER)VALUES
    ('MLB3931', 'Melbourne South East', '123 Demon Street, Mornington, 3931', 'Bruce Wayne');

    INSERT INTO PRODUCT5362(PRODNAME, BUYPRICE, SELLPRICE) VALUES
    ('APPLE ME PHONE X', '890.00', 1295.00 );

    DECLARE @PRODID INT = @@IDENTITY;

    INSERT INTO INVENTORY5362(PRODUCTID, LOCATIONID, NUMINSTOCK) VALUES
    (@PRODID, 'MLB3931', 0);

    -- ADD A NEW CLIENT ACCOUNT AND A NEW AUTHORISED USER FOR THAT ACCOUNT

    INSERT INTO CLIENTACCOUNT5362(ACCTNAME, BALANCE, CREDITLIMIT) VALUES
    ('FREDS LOCAL PHONE STORE', '0', 10000.00 );

    DECLARE @ACCOUNTID INT = @@IDENTITY;

    INSERT INTO AUTHORISEDPERSON5362(FIRSTNAME, SURNAME, EMAIL, [PASSWORD], ACCOUNTID) VALUES
    ...
    INSERT INTO PURCHASEORDER5362(PRODUCTID, LOCATIONID, DATETIMECREATED, QUANTITY, TOTAL) VALUES
    (@PRODID,  'MLB3931', '10-Apr-2020', 50, 44500.00);

    -- UPDATE OUR INVENTORY FOR THAT STOCK
    UPDATE INVENTORY5362 SET NUMINSTOCK = 50 WHERE PRODUCTID = @PRODID AND LOCATIONID = 'MLB3931';

    -- UPDATE THE GENERAL LEDGER INCREASING THE VALUE OF OUR STOCK ASSETS AND DECREASING THE CASH ASSETS
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT - 44500.00 WHERE DESCRIPTION = 'ASSETSCASH';
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT + 44500.00 WHERE DESCRIPTION = 'ASSETSSTOCK';

    -----------

    -- CUSTOMER MAKES AN ORDER - (INITIALLY THE ORDER IS NOT FULFILLED)

    INSERT INTO ORDER5362(SHIPPINGADDRESS, DATETIMECREATED, DATETIMEDISPATCHED, TOTAL, USERID) VALUES
    ('7 Lucky Strike, Bedrock, USB, 1111', '20-Apr-2020', NULL, 6151.25, @USERID);

    DECLARE @ORDERID INT = @@IDENTITY;

    INSERT INTO ORDERLINE5362(ORDERID, PRODUCTID, QUANTITY, DISCOUNT, SUBTOTAL) VALUES
    (@ORDERID, @PRODID, 5, 0.05, '6151.25');

    -- WE FULLFILL THE ORDER

    -- UPDATE THE ORDER TO GIVE IT A FULLFUILLED DATE
    UPDATE ORDER5362 SET DATETIMEDISPATCHED = '21-Apr-2020' WHERE ORDERID = @ORDERID;

    -- UPDATE THE CLIENTS ACCOUNT BALANCE TO INCLUDE THE VALUE OF THE ORDER
    UPDATE CLIENTACCOUNT5362 SET BALANCE = BALANCE + 6151.25 WHERE ACCOUNTID = @ACCOUNTID;

    -- UPDATE THE GENERAL LEDGER INCREASING VALUE OF ACCOUNTS, DECEASING VALUE OF STOCK
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT + 6151.25  WHERE DESCRIPTION = 'ASSETSACCOUNT';
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT - (5*890) WHERE DESCRIPTION = 'ASSETSSTOCK';

    -------------

    -- CLIENT MAKES AN ACCOUNT OFF THIER ACCOUNT BALANCE

    -- ADD A ROW TO ACCOUNTPAYMENT5362
    INSERT INTO ACCOUNTPAYMENT5362(ACCOUNTID, DATETIMERECEIVED, AMOUNT) VALUES
        (@ACCOUNTID, '25-Apr-2020', '2000.00');

    -- UPDATE THE CLIENT ACCOUNT TO REFLECT THE BALANCE CHANGE
    UPDATE CLIENTACCOUNT5362 SET BALANCE = BALANCE - 2000.00 WHERE ACCOUNTID = @ACCOUNTID;

    -- UPDATE THE GENERAL LEDGER - INCREASE ASSETSCASH AND DECREASE ASSETS ACCOUNT
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT + 2000.00 WHERE DESCRIPTION = 'ASSETSCASH';
    UPDATE GENERALLEDGER5362 SET AMOUNT = AMOUNT - 2000.00 WHERE DESCRIPTION = 'ASSETSACCOUNT';
END;
...
BEGIN
    ...
    INSERT INTO LOCATION5362 (LOCID, LOCNAME, LOCADDRESS, MANAGER)
    VALUES(@PLOCID, @PLOCNAME, @PLOCADRESS, @PMANAGER);
...
END;
    ...
    -- insert the specified values into the table LOCATION5362
    -- ADD A ROW FOR THIS LOCATION TO THE INVENTORY5362 TABLE **FOR EACH** PRODUCT IN THE PRODUCT5362 TABLE
...
EXEC ADD_LOCATION @PLOCID = 1, @PLOCNAME = '1', @PLOCADDRESS = 'ADDRESS', @PMANAGER = '';
...
CREATE PROCEDURE GET_LOCATION_BY_ID @PLOCID INT, @PRETURNSTRING NVARCHAR(1000) AS
BEGIN
    BEGIN TRY
    SELECT @PRETURNSTRING =  CONCAT 
        ('LOCATION ID IS:', LOCATIONID,'LOCATION NAME IS:', LOCNAME, 'ADDRESS IS:', LOCADDRESS)
    FROM LOCATION5362
    ...
    BEGIN CATCH
        IF ERROR_NUMBER() = 2647
            THROW 51002, 'LOCATION DOES NOT EXIST (GET_LOCATION_BY_ID, PURCHASE_STOCK', 1;
        ELSE
            BEGIN
                DECLARE @ERRORMESSAGE NVARCHAR(MAX) = ERROR_MESSAGE;
                THROW 50000, @ERRORMESSAGE, 1;
...
BEGIN
    ...
    -- THE FOLLOWING MUST BE COMPLETED AS A SINGLE TRANSACTION
    -- insert the specified values into the table PRODUCT5362
    -- ADD A ROW FOR THIS PRODUCT TO THE INVENTORY5362 TABLE **FOR EACH** LOCTAION IN THE LOCATION5362 TABLE
...
BEGIN
    ...
    -- UPDATE INVENTORY5362 FOR THE SPECIFIED PRODUCT IN THE SPECIFIED LOCATION BY THE QTY PURCHASED
...
BEGIN
    ...
    -- insert the specified values into the table CLIENTACCOUNT5362
...
BEGIN
    ...
    -- insert the specified values into the table AUTHORISEDPERSON5362
...
BEGIN
    ...
    -- THE FOLLOWING MUST BE COMPLETED AS A SINGLE TRANSACTION
    -- insert the specified values into the table ACCOUNTPAYMENT5362 (USING THE CURRENT SYS DATETIME)
    -- UPDATE THE RELEVANT ACCOUNT IN CLENTACCOUNT5362 TO RELFECT THE BALANCE REDUCED BY THE PAYMENT
...
BEGIN
    ...
    -- insert the specified values into the table ORDER5362
    -- SET THE TOTAL TO 0
...
BEGIN
    ...
    -- IF THE PRODUCT HAS NOT ALREADY BEEN ADDED TO THAT ORDER (I.E. PK IS UNIQUE)
        -- insert the specified values into the table ORDERLINE5362
...
BEGIN
    ...
    -- DELETE THE RELEVANT ROW FROM ORDERLINE5362
